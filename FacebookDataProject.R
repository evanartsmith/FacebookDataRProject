...
library(GGally)
setwd("C:/Mydata")
df_fb <- read.csv("pseudo_facebook (1).tsv", sep = "\t")

dob_month <- as.factor(df_fb$dob_month)
...
#friend count histogram, faceted by gender including female/male/unassigned
qplot(x = friend_count, data=df_fb, binwidth = 25)+
  scale_x_continuous(limits = c(0,1000), breaks = seq(0,1000,5)) +
  facet_wrap(~gender)

# applying limits to x to make the x axis more presentable
qplot(x = friend_count, data = df_fb,xlim = c(0,1000), 
...
g3 = qplot(x = sqrt(friend_count), data = df_fb, binwidth = 1, xlim = c(0,1000), xlab = seq(0,1000,50)) 
...
p1 = qplot(x = www_likes, data = df_males, main = "Male")

p2 = qplot (x = www_likes, data = df_females, main = "Female" ) 
...
qplot(x = gender, y = friend_count, data = df_fb, geom = "boxplot")+
